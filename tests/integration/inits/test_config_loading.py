...
@pytest.fixture(name="_unparseable_config")
def unparseable_config_fixture(monkeypatch, tmp_path):
    config_path = tmp_path / "config.toml"
    config_path.write_text(UNPARSEABLE_TOML)
    monkeypatch.setattr(config_module, "DEFAULT_CONFIG_PATH", config_path)
...
class TestLoadConfig:
    ...
    @staticmethod
    def test_a_file_that_does_not_parse_raises(*, tmp_path):
        config_path = tmp_path / "config.toml"
        config_path.write_text(UNPARSEABLE_TOML)
...
class TestStartingWithoutUsableConfig:
    ...
    @staticmethod
    @pytest.mark.usefixtures("_unparseable_config")
    def test_a_file_that_does_not_parse_is_reported_too(*, capsys):
        with pytest.raises(SystemExit):
            main()
